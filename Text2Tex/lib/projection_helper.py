...
def init_renderer2(camera, shader, image_size, faces_per_pixel, blur_radius=0.0):
    raster_settings = RasterizationSettings(
        image_size=image_size, 
        faces_per_pixel=faces_per_pixel,
        blur_radius=blur_radius  # 调整插值程度，可以尝试不同的值
    )
...
def render_one_view2(mesh,
    ...
    #import pdb;pdb.set_trace()
    renderer = init_renderer2(cameras,
        shader=init_soft_phong_shader(
            camera=cameras,
            blend_params=BlendParams(),
            device=device),
...
@torch.no_grad()
def render_one_view_and_build_masks_2(dist, elev, azim, 
    mesh, 
    image_size, faces_per_pixel,
    device,B):
