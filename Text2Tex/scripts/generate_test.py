...
def apply_coloe (label _mask )
vis=np.take(srhgn_coloemay,label_mask ,axis=0)
retuen vis(_.)
...
if __name__ == "__main__":
    ...
    init_texture = Image.open("/data/zeyu_li/Text2Tex/data/test/Bar_Chair_C.jpg").convert("RGB").resize((args.uv_size, args.uv_size))
    ...
    NUM_PRINCIPLE = 10 if args.use_shapenet or args.use_objaverse else 6   #选择10个初始生成视点
    ...
    # #保留原始纹理资产
    # save_backproject_obj(
    #     mesh_dir, "{}_ori.obj".format(view_idx),
    ...
    for view_idx in range(NUM_PRINCIPLE):  #对每个视点进行处理
        ...
        # #添加材质预测图进行UV映射和纹理更新!!!!!!!!!!!!!
        material = get_material(os.path.join(inpainted_image_dir, "{}_before.png".format(view_idx)))
        ...
        if not args.no_update and update_mask_tensor.sum() > 0 and update_mask_tensor.sum() / (all_mask_tensor.sum()) > 0.05:
            ...
            material = get_material(os.path.join(inpainted_image_dir, "{}_update_before.png".format(view_idx)))
    ...
    if args.update_steps > 0:
        ...
        for view_idx in range(args.update_steps):
            ...
            if args.use_patch:
                ...
                material = get_material(os.path.join(inpainted_image_dir, "{}_new_before.png".format(view_idx)))
                ...
                save_backproject_obj(
                    ...
                    mesh.verts_packed(), faces.verts_idx, new_verts_uvs, faces.textures_idx, init_material, 
                    DEVICE
                )
        ...
        if args.post_process:
            ...
            post_texture = apply_inpainting_postprocess(inpainting, 
                init_material, 1-exist_texture[None, :, :, None], "", args.uv_size, args.uv_size, DEVICE)
